...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
  ...
  // Resetea el formulario cuando el modal se abre o se cierra
  useEffect(() => {
    if (isOpen) {
      setFormData({
        nombre_remitente: "",
  ...
  const handleChange = (
    ...
    // Limpia el error para el campo que está siendo modificado
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const API_ENDPOINT = "http://127.0.0.1:5000/mensajecontacto"; // Ajusta si tu Blueprint tiene un prefijo (ej. "/api/mensajecontacto")

      // Mapear los nombres de los campos del frontend a los esperados por el backend
      const payload = {
      ...
      const response = await fetch(API_ENDPOINT, {
        ...
        body: JSON.stringify(payload), // Enviar el payload con los nombres de campo correctos
      });

      const responseData = await response.json(); // Intentar parsear JSON siempre

      if (!response.ok) {
        // Si la respuesta no es OK, el backend debería haber enviado un JSON con 'message' o 'error'
        // responseData.message es lo que tu backend envía para errores 400
        // responseData.error es lo que tu backend envía para errores 500
        throw new Error(
      ...
      // Si la respuesta es OK (ej. 201 Created), responseData contendrá el mensaje de éxito y los datos creados
      console.log("Success:", responseData);
      // Puedes usar responseData.message si quieres mostrar el mensaje del backend
      // Por ejemplo: setSuccessMessage(responseData.message);

      setSubmitStatus("success"); // Mantiene el mensaje de éxito genérico del frontend por ahora
      // o puedes crear un estado para el mensaje de éxito y usar responseData.message

      setTimeout(() => {
        onClose(); // Cierra el modal después de mostrar el mensaje de éxito
      }, 3000);
    } catch (error: any) {
      // Tipar error como 'any' o 'Error' para acceder a 'message'
      console.error("Error al enviar el formulario:", error);
      // Actualiza el estado de error para mostrar el mensaje de error específico
      // Si error.message existe (viene del throw new Error), úsalo.
      // Si no, usa un mensaje genérico.
      //const errorMessageToDisplay =
      //  error.message || getContactModalLabel("errorMessage", currentLanguage);

      // Para mostrar el error específico en el UI, necesitarías un estado para ello, ej. setSubmitErrorMessage(errorMessageToDisplay)
      // Por ahora, se usa el genérico del 'submitStatus'
      setSubmitStatus("error");

      // Si quieres mostrar el error específico en el modal, podrías hacer algo como:
      // setErrors(prev => ({ ...prev, form: errorMessageToDisplay }));
      // Y luego en el JSX mostrar errors.form
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Oculta el formulario si el envío fue exitoso para no reenviar */}
        {submitStatus !== "success" && (
